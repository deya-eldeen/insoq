...
class PlatesFillter: UIView {
    @IBOutlet var contentView: UIView!
    //Sliders
    @IBOutlet weak var digitsTableView: UITableView!
    ...
    var testTypes:[String]=["Private Cars","Classic Cars","Motorcycle"]
    var testDigits:[String]=["1 DIGIT","2 DIGIT","3 DIGIT","4 DIGIT"]
    ...
    private func setDelegates(){
        ...
        digitsTableView.delegate=self
        digitsTableView.dataSource=self
    ...
    private func registerNib(){
        ...
        digitsTableView.register(propertyType, forCellReuseIdentifier: "CheckBox_RadioButtonTableViewCell")
        typesTableView.register(propertyType, forCellReuseIdentifier: "CheckBox_RadioButtonTableViewCell")
    }
    
    func xibSetup() {
        Bundle.main.loadNibNamed("PlatesFillter", owner: self, options: nil)
...
extension PlatesFillter:UITableViewDataSource,UITableViewDelegate{
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if tableView == digitsTableView{
            return  testDigits.count   }
        else if tableView == typesTableView{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView == typesTableView{
            ...
            cell.setNotificationFillterOptions(data:testTypes[indexPath.row])
            return cell

        }else if tableView == serviceTypeTableView {
            let cell = tableView.dequeueReusableCell(withIdentifier: "CheckBox_RadioButtonTableViewCell", for: indexPath ) as! CheckBox_RadioButtonTableViewCell
            cell.setNotificationFillterOptions(data:testFilltered[indexPath.row])
            return cell

        }
        else{
            let cell = tableView.dequeueReusableCell(withIdentifier: "CheckBox_RadioButtonTableViewCell", for: indexPath ) as! CheckBox_RadioButtonTableViewCell
            cell.setNotificationFillterOptions(data:testDigits[indexPath.row])
            return cell
