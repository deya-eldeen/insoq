...
//  Utrac Delivery
//
//  Created by Mr.Marwan Abd-Alhadi on 7/29/20.
//  Copyright © 2020 utrac. All rights reserved.
//
...
extension UIView: Shimmerable {
    ...
    private struct ShimmerProperties {
        ...
        static var lightColor:CGColor = UIColor.white.withAlphaComponent(1).cgColor
        static var darkColor:CGColor = UIColor.black.withAlphaComponent(0.2).cgColor
    ...
    func start(count: Int = 3) {
        ...
        ShimmerProperties.gradient.frame = CGRect(x: CGFloat(-2*self.bounds.size.width), y: CGFloat(0.0), width: CGFloat(5*self.bounds.size.width), height: CGFloat(self.bounds.size.height))
