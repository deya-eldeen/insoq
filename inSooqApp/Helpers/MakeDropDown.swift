...
class MakeDropDown: UIView{
    ...
    func setUpDropDown(viewPositionReference: CGRect,  offset: CGFloat){
        addBorders( borderColor: #colorLiteral(red: 0.6000000238, green: 0.6000000238, blue: 0.6000000238, alpha: 1))
        addShadowToView()
        ...
        self.vwidth = viewPositionReference.width
        self.offset = offset
    ...
    func showDropDown(height: CGFloat){
        if isDropDownPresent{
            ...
            self.frame = CGRect(x: (self.viewPositionRef?.minX)!, y: (self.viewPositionRef?.maxY)! + self.offset, width: vwidth, height: 0)
            self.dropDownTableView?.frame = CGRect(x: 0, y: 0, width: vwidth, height: 0)
            self.dropDownTableView?.reloadData()
            
            UIView.animate(withDuration: 0.7, delay: 0, usingSpringWithDamping: 0.6, initialSpringVelocity: 0.05, options: .curveLinear
                , animations: {
                self.frame.size = CGSize(width: self.vwidth, height: height)
                self.dropDownTableView?.frame.size = CGSize(width: self.vwidth, height: height)
            })
        }
    ...
    func hideDropDown(){
        ...
        UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 0.8, initialSpringVelocity: 0.3, options: .curveLinear
            , animations: {
            self.frame.size = CGSize(width: self.vwidth, height: 0)
            self.dropDownTableView?.frame.size = CGSize(width: self.vwidth, height: 0)
        })
    }
